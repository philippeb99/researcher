-- Add three new financial/company size columns to research_jobs table
ALTER TABLE research_jobs 
ADD COLUMN employee_count INTEGER,
ADD COLUMN revenue_amount DECIMAL(15,2),
ADD COLUMN ebitda_amount DECIMAL(15,2);

COMMENT ON COLUMN research_jobs.employee_count IS 'Number of employees in the company';
COMMENT ON COLUMN research_jobs.revenue_amount IS 'Annual revenue in currency units (USD)';
COMMENT ON COLUMN research_jobs.ebitda_amount IS 'EBITDA (Earnings Before Interest, Taxes, Depreciation, and Amortization) in currency units (USD)';
